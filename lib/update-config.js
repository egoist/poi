...
module.exports = co.wrap(function * (webpackConfig, options) {
  ...
  if (options.dev) {
    ...
    if (options.stats) {
      ...
      webpackConfig.plugins.push(new Visualizer(options.stats))
    }
  ...
  // add alias
  if (options.aliasDefault) {
    const srcPath = typeof options.aliasDefault === 'string'
      ? options.aliasDefault
      : 'src'
  ...
  if (options.resolveRoot) {
    const resolveRoot = options.resolveRoot === true ? 'src' : options.resolveRoot
    webpackConfig.resolve.modules.push(_.cwd(resovleRoot))
  }
